...
if __name__ == "__main__":
    ...
    sensor1 = MCP23S17Sensor(bus=args.bus, ce=args.ce, deviceID=args.dev)
    
    # move srcp to the non-standard port
    srcpSock = SRCPSock('localhost', args.srcpdport)
    srcpSock.connect()

    # make proxy available on srcp standard port
    proxy = RocrailProxy('localhost', args.reqport, sensors=[ sensor1 ])
